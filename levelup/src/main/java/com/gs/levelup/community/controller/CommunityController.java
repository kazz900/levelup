...
public class CommunityController {
	...
	public String deleteCommunity(
			String boardId, Model model, int currentPage
			) {
	...
	public String insertCommunityArticle(Community community, Model model, 
			int currentPage,
			HttpServletRequest request, @RequestParam(name="ofile",required=false) MultipartFile mfile) {
...
//		community.setRenameFilePath(renameFileName);
	...
	public ModelAndView selectCommunity(@RequestParam("boardNo") int boardNo , 
		...
		public ModelAndView noticeSearchDateMethod(
			...
			if (list != null && list.size() > 0) {
				...
				mv.addObject("message", action + "에 대한" + searchDate.getBegin() + "부터 " 
						+ searchDate.getEnd() + " 기간 사이의 검색 결과가 존재하지 않습니다.");
				mv.setViewName("common/error");
			}
			return mv;
		}
