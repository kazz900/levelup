...
public class EmployeeController {
	//이 컨트롤러 안의 메소드들이 구동되는지, 값이 잘 전달되었는지 확인을 위한 로그 객체를 생성
		...
		public ModelAndView employeeSearchMethod(HttpServletRequest request, ModelAndView mv) {
			...
			int listCount = 0;
			switch(action) {
			case "id": listCount = employeeService.selectSearchIDCount(keyword); break;
			case "gender": listCount = employeeService.selectSearchGenderCount(keyword); break;
			case "age": listCount = employeeService.selectSearchAgeCount(Integer.parseInt(keyword)); break;
			case "enrolldate": listCount = employeeService.selectSearchDateCount(new SearchDate(Date.valueOf(begin), Date.valueOf(end))); break;
			case "loginok": listCount = employeeService.selectSearchLoginOKCount(keyword); break;
			}
