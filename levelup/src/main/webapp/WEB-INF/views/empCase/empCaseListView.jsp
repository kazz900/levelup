							...
							<div class="inputs">
								...
								<div class="typeDropdownContainer">
									<select id="searchByTypeSelect" name="type">
										<option value="0">기안구분</option>
										<option value="1">아이템 변경</option>
										<option value="2">아이템 삭제</option>
										<option value="3">아이템 환불</option>
									</select>
								</div>
								<input id="keywordInputbox" type="search" name="keyword">
								<input id="searchSubmitButton" type="submit" value="SEARCH">
								<input type="button" onclick="javascript:location.href='${ pageContext.servletContext.contextPath }/clist.do?page=${ nowpage }'" value="RESET">
							</div>
						</div>
					</form>
					<!-- 문의 테이블 -->
							...
							<c:forEach items="${ requestScope.list }" var="c">
								<tr>
									...
									<c:url var="cdetail" value="cdetail.do">
										...
										<c:param name="originalItemName" value="${ c.originalItemName }"/>
										<c:param name="newItemName" value="${ c.newItemName }"/>
									</c:url>
